...
def main(subs, skips) -> None:
    ...
    for (fpath, sub, task, run) in iter_BIDSPaths(fpaths):
        ...
        if sub in skips:
            continue
        print("subprocess.check_call(\"sbatch ./coherence.py %s %s %s %s\" % (fpath, sub, task, run), shell=True)")
        subprocess.check_call("sbatch ./coherence.py %s %s %s %s" % (fpath, sub, task, run), shell=True)
